...
def file_based_input_fn_builder(input_file, seq_length, is_training, drop_remainder):
    ...
    def _decode_record(record, name_to_features):
        ...
        # tf.Example 仅支持 tf.int64，但 TPU 仅支持 tf.int32。将所有 int64 转换为 int32。
    ...
    def input_fn(params):
        ...
        # 对于训练，我们需要大量的并行读取和数据混洗。对于评估，不需要混洗，并行读取无关紧要。
        if is_training:
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
    ...
    output_bias = tf.Variable(
        initial_value=tf.zeros([num_labels]),
        name="output_bias"
    )
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    if init_checkpoint:
      ...
      if use_tpu:
          ...
          # Initialize from checkpoint without TPU
          tf.compat.v1.train.init_from_checkpoint(init_checkpoint, assignment_map)
          scaffold_fn = None
    ...
    if mode == tf.compat.v1.estimator.ModeKeys.TRAIN:
      ...
      def metric_fn(per_example_loss, label_ids, logits, is_real_example):
          ...
          # 使用 tf.compat.v1.metrics 来保持和 TF 1.x 相兼容
      ...
      # 使用 tf.estimator.tpu.TPUEstimatorSpec 替代 tf.contrib.tpu.TPUEstimatorSpec
      output_spec = tf.compat.v1.estimator.tpu.TPUEstimatorSpec(
          mode=mode,
          loss=total_loss,
