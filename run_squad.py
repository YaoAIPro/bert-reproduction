...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
  ...
  output_weights = tf.Variable(
      initial_value=tf.random.truncated_normal([2, hidden_size], stddev=0.02),
      name="cls/squad/output_weights"
  )

  output_bias = tf.Variable(
      initial_value=tf.zeros([2]),
      name="cls/squad/output_bias"
  )
...
def input_fn_builder(input_file, seq_length, is_training, drop_remainder):
  ...
  def input_fn(params):
    ...
    d = d.map(
        ...
        num_parallel_calls=tf.data.experimental.AUTOTUNE  # 并行处理
    )
...
def main(_):
  ...
  run_config = tf.compat.v1.estimator.tpu.RunConfig(
      ...
      tpu_config=tf.compat.v1.estimator.tpu.TPUConfig(
          ...
          per_host_input_for_training=is_per_host  # 代替 is_per_host
      )
  )
  ...
  if FLAGS.do_train:
    ...
    logging.info("  Num split examples = %d", train_writer.num_features)
